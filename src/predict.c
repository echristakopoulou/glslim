...
/*! \file predict.c
    ...
    computing training error, used by GLSLIM and GLSLIMr0.
...
/**************************************************************/
/*! \brief Top-N recommendation and evaluations
    
    \param[in] ctrl          A ctrl structure
    \param[in] model         A model
    \param[in] g             A vector specifying the weight of every user.
	       ...
	       gk_csr_t * train, gk_csr_t * test, int *participation)
{
  ...
  for (u = startu; u < endu; u++) {
    ...
    nrcmd = recommend(ctrl, model, g[u], iidx, train, u, &rcmd,
		      participation[u]);
...
/*! \brief Top-N recommendation for a user
    ...
    \param[in] g_participation The weight of this specific user.
	      ...
	      gk_csr_t * train, int u, gk_dkv_t ** rcmd, int cluster)
{
  ...
  global = all_predict(ctrl, model, train, u, g_participation, cluster);
...
/*! \brief Compute all possible predictions for user u for all items.
  ...
  \param[in] g_participation  The weight of user u.
...
double *all_predict(ctrl_t * ctrl, gk_csr_t * model, gk_csr_t * train,
		    int u, double g_participation, int cluster_id)
{
  ...
  for (i = train->rowptr[u]; i < train->rowptr[u + 1]; i++) {
    ii = train->rowind[i];
    fixed_rcmd_part = g_participation * train->rowval[i];
    for (j = model->colptr[ii]; j < model->colptr[ii + 1]; j++) {
      jj = model->colind[j];
  ...
  for (i = train->rowptr[u]; i < train->rowptr[u + 1]; i++) {
    ii = train->rowind[i];
    fixed_rcmd_part = (1 - g_participation) * train->rowval[i];
    index = (ctrl->size) * (cluster_id + 1) + ii;
...
/*********************************************************************/
/*! \brief Compute all possible predictions for user u for all items,
                without any user weight.
  \param[in]  ctrl             Ctrl structure.
  \param[in]  model            The model.
  \param[in]  train            Training data.
  \param[in]  u                The user in question.
  \param[out] global_rcmd      The vector of the global prediction values. 
  \param[out] local_rcmd       The vector of the local prediction values.
  \param[in]  cluster_id       The cluster assignment of user u.
		   ...
		   int cluster_id)
{
  ...
  for (i = train->rowptr[u]; i < train->rowptr[u + 1]; i++) {
    ...
    for (j = model->colptr[ii]; j < model->colptr[ii + 1]; j++) {
      jj = model->colind[j];
      (*global_rcmd)[jj] += fixed_rcmd_part * model->colval[j];
    }
  }
  ...
  for (i = train->rowptr[u]; i < train->rowptr[u + 1]; i++) {
    ...
    for (j = model->colptr[index]; j < model->colptr[index + 1]; j++) {
      jj = model->colind[j];
      (*local_rcmd)[jj] += fixed_rcmd_part * model->colval[j];
    }
  }
...
/*! \brief Compute the training error for a specific user.
  ...
  \param[in] model           The model.
  \param[in] g_participation The user's weight.
...
/*! \brief Compute the training error for all users.
  ...
  \param[in] train          The training data.
  \param[in] model          The model.
  \param[in] g              The weights of all users.
  \param[in] participation  The clustering assignment vector of all users.
  
  \return    overall_error  The overall training error.
*/
/******************************************************************************/
		     ...
		     double *g, int *participation)
{
  ...
  for (u = startu; u < endu; u++) {
    error = training_error(ctrl, model, g[u], train, u, participation[u]);
    total_error += error;
  }
