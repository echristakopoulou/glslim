/**************************************************************/
/*! \file main_lslim.c
    \brief This is the file containing the main entry for LSLIM.
...
int main(int argc, char *argv[])
{
  ...
  if (iter != 0) {
    ...
    if ((ctrl->size) * (ctrl->num_clusters) > prev_model->ncols)
      prev_model->ncols = (ctrl->size) * (ctrl->num_clusters);
    ...
    model = lslim_learn(ctrl, train, participation, prev_model);
    if (prev_model != NULL) {
    ...
    /* Compute training error */
      error = lslim_train_predict(ctrl, train, model, participation);
   ...
   lslim_test(ctrl, model, train, test, participation);
   ...
   participation = lslim_learn_pu_all(ctrl, train, model, participation2, 
				      &indifference);
  ...
  gk_csr_Free(&train);
  gk_csr_Free(&test);
  gk_free((void **) &participation, &indifference, LTERM);
  if(ctrl->id==0){
