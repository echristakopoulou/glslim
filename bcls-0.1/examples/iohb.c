...
/*************************************************/
    ...
    upcase(fmt);
    tmp2 = strchr(fmt,'(');
    tmp  = substr(fmt,tmp2 - fmt + 1, strchr(fmt,'I') - tmp2 - 1);
    *perline = atoi(tmp);

    free(tmp);

    tmp2 = strchr(fmt,'I');
    tmp  = substr(fmt,tmp2 - fmt + 1, strchr(fmt,')') - tmp2 - 1);
    *width = atoi(tmp);
...
/*************************************************/
    ...
    *perline = atoi(tmp);
    ...
    tmp2 = strchr(fmt,*flag);
    if ( strchr(fmt,'.') ) {
      tmp = substr( fmt, strchr(fmt,'.') - fmt + 1, strchr(fmt,')') - strchr(fmt,'.')-1);
      *prec = atoi( tmp );
      free(tmp);
      tmp = substr(fmt,tmp2 - fmt + 1, strchr(fmt,'.') - tmp2 - 1);
    } else {
