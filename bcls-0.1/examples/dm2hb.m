...
% Default format
if nargin < 5 || isempty(eval('key'))
    key = 'key';
    if nargin < 4 || isempty(eval('key'))
     	title = 'title';
    end;
end;
if nargin < 6 || isempty(eval('type'))
    type = 'RUA';
end;
if nargin < 7 || isempty(eval('ifmt'))
    ifmt = 8;
end;
if nargin < 8 || isempty(eval('job'))
    job = 2;
end;
